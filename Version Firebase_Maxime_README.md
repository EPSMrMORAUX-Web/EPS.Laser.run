...
Le projet utilise **Firebase** pour la synchronisation des résultats entre appareils via un code de session.

### Projet Firebase

| Paramètre | Valeur |
|---|---|
| **Project ID** | `eps-laser-run` |
| **Auth Domain** | `eps-laser-run.firebaseapp.com` |
| **Database URL** | `https://eps-laser-run-default-rtdb.europe-west1.firebasedatabase.app` |
| **Storage Bucket** | `eps-laser-run.firebasestorage.app` |
| **Messaging Sender ID** | `436261236946` |
| **App ID** | `1:436261236946:web:3ce7c9f58894bb8e3f7d0b` |

### Bloc `firebaseConfig` à copier dans le HTML

```javascript
const firebaseConfig = {
  apiKey: "AIzaSyBYgDs4wRy0pgOGY0nKJC8WHGHK5rHQrgc",
  authDomain: "eps-laser-run.firebaseapp.com",
  databaseURL: "https://eps-laser-run-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "eps-laser-run",
  storageBucket: "eps-laser-run.firebasestorage.app",
  messagingSenderId: "436261236946",
  appId: "1:436261236946:web:3ce7c9f58894bb8e3f7d0b"
};
```

### Installation du SDK Firebase (via npm)

```bash
npm install firebase
```

### Initialisation dans le fichier HTML
...
> ⚠️ **Sécurité** : La clé API Firebase est visible dans le code source HTML. Protégez votre base de données en configurant des **règles de sécurité Firebase** (Realtime Database → Règles) pour limiter les accès en lecture/écriture, notamment par code de session.

### Règles Firebase recommandées (mode EPS)
...
Double-cliquez sur le fichier `laser-run-eps-v5.html` dans n'importe quel navigateur moderne. Aucune connexion internet nécessaire (sauf pour les polices Google Fonts et Firebase au premier chargement).
...
- Si une session est active, le résultat est **automatiquement enregistré** dans Firebase sous le code de session
...
> Avec Firebase, cette méthode fonctionne sur **tous les appareils**, même des téléphones différents, sans partager le même navigateur.
...
Élève → Valide → Note calculée + QR code affiché
         └─ Résultat envoyé automatiquement sur Firebase

RÉCUPÉRATION
─────────────
Option 1 — Enseignant → Session → Importer → code "1234" → tous les résultats (via Firebase)
...
### localStorage (local, par appareil)

| Clé | Contenu |
|---|---|
| `laserrun_v5` | Élèves et passages de l'appareil courant |
| `lr_session_XXXX` | Résultats associés au code de session XXXX (cache local) |
...
### Firebase Realtime Database (partagé, multi-appareils)

| Chemin | Contenu |
|---|---|
| `/sessions/XXXX/` | Résultats de tous les élèves du code de session XXXX |

> Les données Firebase persistent jusqu'à suppression manuelle depuis la console Firebase.
...
| [Firebase JS SDK](https://firebase.google.com/docs/web/setup) v9+ | Synchronisation des sessions | CDN Firebase |
...
- La clé API Firebase est **visible dans le code source** — c'est normal pour une application web Firebase. Protégez vos données via les **règles de sécurité** de la Realtime Database (voir section Firebase ci-dessus).
